...
const Service = ({ active }: { active: boolean }) => {
  ...
  // Loading animation for the Service component
  useEffect(() => {
    if (!active) return;
    
    const startTime = Date.now();
    const duration = 2500;
    const totalLetters = 7; // "Service" has 7 letters
    ...
    const animateLetters = () => {
      ...
      setLoadingProgress(lettersCompleted);
  ...
  // Auto-scroll to bottom on load when active and loaded
  useEffect(() => {
  ...
  useEffect(() => {
    if (!active) {
      ...
      // Reset animations
      if (bellRef.current) {
        gsap.set(bellRef.current, { y: 0 });
      }
      if (ropeRef.current) {
        gsap.set(ropeRef.current, { height: 0 });
      }
      if (containerRef.current) {
        gsap.set(containerRef.current, { scrollTo: { y: 0 } });
      }
    }
  }, [active]);
  ...
  // Only render content when active
  if (!active) {
    return (
